...
function findPrimes(min, max, cb) {
  return new Promise((resolve, reject) => {
    ...
    if (cb === undefined) cb = () => {}; //define a dummy callback
...
function findPrimesInteractive(min, max, cb) {
  if (cb === undefined) throw new Error("callback not specified");
  ...
  function conclude(message) {
    if (message === "abort") {
      clearInterval(iid);
      cb({ message: "aborted", id, min, max, done: high - min, primes: index });
      return "aborted";
    }
  }
