...
class CLI {
  constructor(appName = "CLI", options = {}) {
    ...
    this.addCommand(
      this.help.bind(this),
      "help",
      "Provides Help for the available commands",
      "?",
      "h"
    );
    this.addCommand(
  ...
  addCommand(command, primaryName, helpText, ...aliases) {
    if (!primaryName) {
      if (command.name) primaryName = command.name;
      else throw new Error("No command name found");
    }
  ...
  async runCommand(command, args) {
    ...
    if (!this.interactiveMode) await this.options.init();
  ...
  help(commandName) {
    function getArgs(command) {
      ...
      if (usageText) return usageText.replace(",", " ");
      else return "";
    }
    ...
    if (commandName) {
      ...
      for (var commandName in this.commands) {
        var command = this.commands[commandName];
        if (!command.isPrimary) continue;
        helpText += `${command.primaryName} ${getArgs(command)}\t${
          command.helpText
        }\tAliases:${command.aliases.join(",")}\n`;
      }
      return helpText;
